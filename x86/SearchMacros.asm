...
._7skip:
  ...
  if PvNode = 0
	...
	; && (ss->ply >= nmp_ply || ss->ply % 2 == pair)) 
		...
		mov   eax, r12d
		and   eax, 1 ; ss->ply % 2
		cmp   al, byte[rbp-Thread.rootPos+Thread.pair]
		jne   .8skip
		shr   cx, 8
		cmp  r12b, byte[rbp-Thread.rootPos+Thread.nmp_ply]
		jl   .8skip
    if USE_MATEFINDER =	0
	      movzx   ecx, word[rbx+State.npMaterial+2*rcx]
	       test   ecx, ecx
...
.8do:
	...
	; r10w holds cpp int values for later
		...
		;     "pair"	 =  lower 8 bits
		mov   r9b, byte[rbp-Thread.rootPos+Thread.nmp_ply]
		mov   r10b, byte[rbp-Thread.rootPos+Thread.pair]
		imul  eax, esi, 3
		sar   eax, 2
		add   eax, r12d
...
.8check:
		...
		;xor   r9d, r9d
	       call   r12
		mov   byte[rbx+State.skipEarlyPruning],	0
	; Now, reset "pair" and "nmp_ply" using the value previously stored in r9b and r10b
	; thisThread->pair = pair
		mov   byte[rbp-Thread.rootPos+Thread.pair], r9b
		;shr   r10w, 8
